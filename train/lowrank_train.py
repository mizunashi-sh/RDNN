...
def build_model(config):
    activation = get_activation(config.activation_name)
    input_size = 1 if config.task_name == "angular_velocity" else 3

    base_model = LowRankRDNN(
    ...
    if config.task_name == "angular_velocity":
        return AngularVelocityNetwork(base_model, config.hidden_size)
    return base_model
...
def train_one_run(config, output_dir):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        print(
            f"[{config.task_name} | LowRankRDNN | {config.activation_name} | H={config.hidden_size} | R={config.rank} | seed={config.seed}] "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train LowRankRDNN on angular velocity and memory-guided saccade tasks."
    )
    parser.add_argument("--output-dir", type=Path, default=Path("results/contrast_train_lowrank"))
    parser.add_argument(
        "--tasks",
        nargs="*",
        default=["angular_velocity", "saccade"],
        choices=["angular_velocity", "saccade"],
    )
    ...
    parser.add_argument("--ranks", nargs="*", type=int, default=[8, 16, 24, 32])
    parser.add_argument("--seeds", nargs="*", type=int, default=list(range(10)))
    return parser.parse_args()


def build_config(args, task_name, activation_name, hidden_size, rank, seed):
...
def main():
    ...
    summaries = []
    for task_name, activation_name, hidden_size, rank, seed in product(
        args.tasks,
        args.activations,
        args.hidden_sizes,
        args.ranks,
        ...
        config = build_config(args, task_name, activation_name, hidden_size, rank, seed)
