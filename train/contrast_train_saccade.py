...
from dataset import SaccadeDataset
...
def build_model(model_name, hidden_size, activation_name, use_noise, noise_std):
    ...
    if model_name == "RDNN":
        return RDNN(**base_kwargs)
    if model_name == "TaskGRU":
        return TaskGRU(**base_kwargs)
    if model_name == "TaskLSTM":
        return TaskLSTM(**base_kwargs)
...
def train_one_run(config, output_dir):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        perm = torch.randperm(config.train_samples)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train RDNN, TaskGRU, and TaskLSTM on the memory-guided saccade task.")
    parser.add_argument("--output-dir", type=Path, default=Path("results/contrast_train_saccade"))
